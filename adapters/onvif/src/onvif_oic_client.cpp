      ...
      if(eCode == OC_STACK_OK)
        {
          ...
          // rep.getValue("power", mylight.m_state);
          // rep.getValue("rtsp_uri", mylight.m_rtsp_uri);
          // rep.getValue("name", mylight.m_name);

          // std::cout << "\tpower: " << mylight.m_state << std::endl;
          // std::cout << "\tname: " << mylight.m_name << std::endl;
          // std::cout << "\trtsp_uri: " << mylight.m_rtsp_uri << std::endl;

          for (const auto& attr : rep) {
            std::cout << attr.attrname() + ": " << rep.getValueToString(attr.attrname())
                      << " [type] :" << attr.type()
                      << std::endl;
          }
      ...
      if(resource)
        {
          ...
          if(resourceURI == "/wto/cso/sto/cam")
            {
  ...
  try
    {
      ...
      // Find all resources
      requestURI << OC_MULTICAST_DISCOVERY_URI << "?rt=core.ipcam";
